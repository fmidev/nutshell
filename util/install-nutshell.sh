...
TOMCAT=''
if [ ${NUTSHELL_VERSION:0:6} == 'tomcat' ]; then
...
# Taking back-up
export NUTSHELL_CONF=${NUTSHELL_JAR_DIR}/${CONF_FILE##*/}
show_variable NUTSHELL_CONF
...
if [ "$TOMCAT" != '' ]; then
	...
	cp -viu ${PRODUCT_EXAMPLES} $HTML_ROOT/product-examples.json
    fi
