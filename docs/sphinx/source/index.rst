...
NutShell is a Python-based facility for generating product files: images, data files and text files.
...
if a product (or "target") needs inputs ("dependencies"), they are generated first. 
The applied *product generators* are organized systematically in a directory tree.
Each product has an ID of type ``my.data.prod`` that is associated with path component ``my/test/prod/``.
Hence, a product generator can be tracked to -- and in a way also identified with -- a unique
subdirectory, which also contains a *generator script* with a fixed name (``generate.sh``).

Typically, Unix/Linux commands have a form like: ::

  command -i <input-file>  -p <parameter> -o <output-file>

Essentially, generator script is a wrapper for such commands.
...
# Installation: https://github.com/fmidev/nutshell
